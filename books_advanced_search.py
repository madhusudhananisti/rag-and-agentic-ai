...
def main():
    try:
        collection = client.create_collection(
            ...
            metadata={"description": "A collection for book information data"},
        ...
        books_documents = []
        for book in books:
            document = f"Book with title: {book['title']} and description: {book['description']} are with themes {book['themes']}. "
            document += f"Settings: {book['setting']}. rating: {book['rating']} and pages: {book['pages']}"
            document += f"Published year: {book['year']} whose author: {book['author']}."
            books_documents.append(document)
        
        #add book to documents and its metadata to collection
        collection.add(
...
def perform_advanced_search(collection, all_items):
    try:
        ...
        query_text = "Books that describes a magical fantasy adventure"
        ...
        for i, (doc_id, document, distance) in enumerate(zip(
            ...
            print(f"     Description: {metadata['description']}, genre: {metadata['genre']}")
            print(f"     Document: {document[:100]}...")

        # Example 1: Filter by department
        print("\n3. Finding all Science Fiction books:")
        results = collection.get(
            where={"genre": "Science Fiction"}
        )
        print(f"Found {len(results['ids'])} Science Fiction books:")
        for i, doc_id in enumerate(results['ids']):
            metadata = results['metadatas'][i]
            print(f"  - {metadata['genre']}: {metadata['author']} ({metadata['year']} , year of publication)")
